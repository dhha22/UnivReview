...
public class RegisterUnivInfoFragment extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        departmentTxt.setOnClickListener(v -> Navigator.goSearch(getActivity(), "department", 1, ""));
        majorTxt.setOnClickListener(v -> Navigator.goSearch(getActivity(), "major", 1, ""));
    ...
    public void onActivityResult(ActivityResultEvent activityResultEvent){
        if(activityResultEvent.getResultCode() == getActivity().RESULT_OK
                && activityResultEvent.getRequestCode() == Navigator.SEARCH){
            Logger.v("data");
        }
    }
